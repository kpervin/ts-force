...
async function checkVersion(): Promise<void> {
    ...
    if (genVersion !== tsforceVersion) {
        console.warn(
            `The version of ts-force-gen (${genVersion}) should match ts-force (${tsforceVersion}). It is recommended that you run \`npm install -D ts-force-gen@${tsforceVersion}\` and regenerate classes`
        );
    }
}
